...
(()=>{
  ...
  const pageMap={
    'index.html':['Home','CTE Network'], 'game-day.html':['Game Day','Live Command Center'], 'changes.html':['Changes','League Activity'], 'news.html':['Newsroom','CTE Bot'], 'article.html':['Newsroom','CTE Bot Article'],
    'scores.html':['Scores','Weekly Matchups'], 'schedule.html':['Schedule','Matchup Center'], 'matchup.html':['Matchup','Game Center'],
    'rivalries.html':['Rivalries','Annual Series'], 'rivalry.html':['Rivalry','Head-to-Head'], 'standings.html':['Standings','League Table'], 'teams.html':['Teams','The 12'],
  ...
  const group=(file==='index.html'||file==='')?'home':(['game-day.html','scores.html','schedule.html','matchup.html'].includes(file)?'gameday':file==='standings.html'?'standings':(['news.html','article.html'].includes(file)?'news':'more'));
  const bottom=document.createElement('nav');bottom.className='app-bottom-nav';bottom.setAttribute('aria-label','Primary');bottom.innerHTML=`
    ...
    <a href="game-day.html" class="${group==='gameday'?'active':''}"><span class="nav-icon">🏈</span><span>Game Day</span></a>
  ...
  if(file==='game-day.html') tabs([['Scoreboard','#scoreboard'],['Lineups','#readiness'],['Changes','changes.html'],['Newsroom','news.html']]);
  if(file==='changes.html') tabs([['Everything','#changeFeed'],['Transactions','transactions.html'],['Newsroom','news.html']]);
  if(file==='scores.html') tabs([['This Week','#content'],['Game Day','game-day.html'],['Full Schedule','schedule.html']]);
  if(file==='schedule.html') tabs([['Schedule','#content'],['Weekly Scores','scores.html'],['Rivalries','rivalries.html']]);
  if(file==='rivalries.html'||file==='rivalry.html') tabs([['Rivalries','rivalries.html'],['Schedule','schedule.html'],['Records','records.html']]);
  if(file==='news.html') tabs([['Newsroom','#newsGrid'],['Game Day','game-day.html'],['Changes','changes.html']]);
