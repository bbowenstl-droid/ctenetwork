// CTE League UI v14 - universal owner/team profile links + live sync
(function(){
'use strict';
...
function nav(active){
 const items=[['index.html','HQ','home'],['scores.html','Scores','scores'],['schedule.html','Schedule','schedule'],['standings.html','Standings','standings'],['teams.html','Teams','teams'],['transactions.html','Transactions','transactions'],['records.html','Records','records'],['league-history.html','History','history'],['draft-central.html','Draft','draft']];
...
function profileHref(id){return `team.html?owner=${encodeURIComponent(id||'')}`}
function isActiveOwner(id){return !!(id&&data()?.owners?.[id]?.status==='active')}
function ownerLink(id,label,cls='profile-link owner-profile-link'){const text=esc(label??owner(id).name);return isActiveOwner(id)?`<a class="${cls}" href="${profileHref(id)}">${text}</a>`:text}
function teamLink(id,label,cls='profile-link team-profile-link'){const text=esc(label??owner(id).currentTeamName??'');return isActiveOwner(id)?`<a class="${cls}" href="${profileHref(id)}">${text}</a>`:text}
...
window.CTE_UI={owner,divisionName,esc,nav,header,shellStart,shellEnd,mapForSeason,standingsRows,fmt,profileHref,isActiveOwner,ownerLink,teamLink,liveControls,setLiveStamp};
})();
